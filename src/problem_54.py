...
def read_file(file_name: str) -> (list, list):
    ...
    with open(file_name, 'r') as f:
        for line in f:
            ...
            first_hand_cards = [Card(card[0], card[1]) for card in cards[0:5]]
            second_hand_cards = [Card(card[0], card[1]) for card in cards[5:10]]

            first_player_cards.append(Hand(first_hand_cards))
            second_player_cards.append(Hand(second_hand_cards))
...
def solve(file_name: str) -> (int, int):
    first_player_hands, second_player_hands = read_file(file_name)
...
def main():
    first_player_wins, second_player_wins = solve('problem_54_poker.txt')
